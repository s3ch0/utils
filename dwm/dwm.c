...
void focusstack(const Arg *arg) {
  ...
  FILE *fp = fopen("/tmp/focusstack", "a");
  fprintf(fp, "focusstack init successful!\n");
  fclose(fp);

  // bug in the focuswin() function !
  if (issinglewin(arg)) {
    FILE *fp = fopen("/tmp/focusstack", "a");
    fprintf(fp, "issinglewin(arg) is true!\n");
    fclose(fp);
    // focuswin(arg);
    FILE *fp2 = fopen("/tmp/focusstack", "a");
    fprintf(fp2, "focuswin(arg) successful\n");
    fclose(fp2);
    return;
  }
